#!/usr/bin/env python3
"""Process CAPTIVE_PORTAL files from minisforum/docs."""
...
def parse_file(filepath):
    ...
    # Category
    category = 'learning'
    if 'AUTH' in filename.upper():
        category = 'setup'
    elif 'OPTIMIZATION' in filename.upper():
        category = 'decision'
    
    # Tags
    tags = ['minisforum', 'captive-portal', 'documentation']
    if 'AUTH' in filename.upper():
        tags.append('authentication')
    if 'OPTIMIZATION' in filename.upper():
...
def main():
    ...
    print(f"Found {len(matched_files)} CAPTIVE_PORTAL files\n")
