#!/usr/bin/env python3
"""Process TRAFFIC, PACKET, ANALYSIS, CHANGES files from minisforum/docs."""
...
PATTERNS = [
    r'TRAFFIC', r'PACKET', r'ANALYSIS', r'CHANGES',
    r'CAPTIVE.*LOG', r'MTU.*ANALYSIS', r'NVRAM.*ANALYSIS',
    r'BRIDGE.*CHANGES'
]
...
def parse_file(filepath):
    ...
    category = 'learning'
    if 'CHANGES' in filename.upper():
        category = 'decision'
    elif 'ANALYSIS' in filename.upper():
        category = 'learning'
    elif 'TRAFFIC' in filename.upper() or 'PACKET' in filename.upper():
        category = 'learning'
    
    # Tags
    tags = ['minisforum', 'documentation']
    if 'TRAFFIC' in filename.upper():
        tags.append('traffic-analysis')
    if 'PACKET' in filename.upper():
        tags.append('packet-loss')
    if 'ANALYSIS' in filename.upper():
        tags.append('analysis')
    if 'CHANGES' in filename.upper():
        tags.append('changes')
    if 'CAPTIVE' in filename.upper():
        tags.append('captive-portal')
    if 'MTU' in filename.upper():
        tags.append('mtu')
    if 'NVRAM' in filename.upper():
        tags.append('nvram')
    if 'BRIDGE' in filename.upper():
        tags.append('bridge-ap')
    
    # Timestamp from file modification
    try:
