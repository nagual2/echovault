#!/usr/bin/env python3
"""Process VHT40 files from minisforum/docs."""
...
def parse_file(filepath):
    ...
    # Category
    category = 'learning'
    if 'GUIDE' in filename.upper() or 'APPLIED' in filename.upper():
        category = 'setup'
    elif 'SCRIPTS' in filename.upper():
        category = 'decision'
    
    # Tags
    tags = ['minisforum', 'vht40', 'wifi', 'documentation']
    if 'DEBIAN' in filename.upper():
        tags.append('debian')
    if 'GUIDE' in filename.upper():
        tags.append('guide')
    if 'SCRIPTS' in filename.upper():
        tags.append('scripts')
    if 'QUICK' in filename.upper():
...
def main():
    ...
    print(f"Found {len(matched_files)} VHT40 files\n")
